...
def astar_search(labyrinth, start, goal, heuristic):
...
path, iterations = astar_search(laberinto, start_node, goal_node, {(i, j): 0 for i in range(len(laberinto)) for j in range(len(laberinto[0]))})
end_time = time.time()
