...
# Calcular la heurística de Manhattan para cada posición en el laberinto
heuristic = {(i, j): manhattan_distance((i, j), goal_node) for i in range(len(laberinto)) for j in range(len(laberinto[0]))}

# Ejecutar Greedy Best-First Search con heurística de Manhattan
start_time = time.time()
path, iterations = greedy_best_first_search(laberinto, start_node, goal_node, heuristic)
end_time = time.time()
