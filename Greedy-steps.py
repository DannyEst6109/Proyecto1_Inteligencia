...
# Función para realizar Greedy Best-First Search en tiempo real
def greedy_best_first_search_real_time(labyrinth, start, goal, heuristic):
...
# Ejecutar Greedy Best-First Search en tiempo real
start_time = time.time()
path, iterations = greedy_best_first_search_real_time(laberinto, start_node, goal_node, {(i, j): 0 for i in range(len(laberinto)) for j in range(len(laberinto[0]))})
end_time = time.time()
