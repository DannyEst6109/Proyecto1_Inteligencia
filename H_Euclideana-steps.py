...
# Función para realizar A* con heurística de Distancia Euclidiana en tiempo real
def astar_search_euclidean_real_time(labyrinth, start, goal):
        ...
        if current not in visited:
            ...
            for neighbor in neighbors:
                if neighbor not in visited:
                    ...
                    heapq.heappush(priority_queue, (new_cost + euclidean_distance(neighbor, goal), new_cost, neighbor, path + [neighbor]))
            ...
            plt.pause(0.01)  # Pausa más corta para una actualización más rápida
...
# Ejecutar A* con heurística de Distancia Euclidiana en tiempo real
start_time = time.time()
path, iterations = astar_search_euclidean_real_time(laberinto, start_node, goal_node)
end_time = time.time()
